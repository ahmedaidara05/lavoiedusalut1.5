// chatbotResponses.js
const chatbotResponses = [
    {
        keywords: ["spiritualité", "chapitre 1", "voie du salut"],
        response: "Dans le chapitre 1 de *La Voie du Salut*, Ahmed Said Aidara invite à une quête spirituelle profonde, encourageant la découverte de soi et la connexion avec le divin à travers la méditation de la parole de Dieu. Il met l’accent sur l’émerveillement, la transformation et la libération par la sagesse de l’Islam."
    },
    {
        keywords: ["vérité", "vérification", "chapitre 1"],
        response: "L’auteur, dans le chapitre 1, insiste sur l’importance de vérifier chaque argument pour en apprécier la véracité, comme il l’écrit : « Je suis responsable de ce que j’écris, mais pas de votre compréhension. » Il s’inspire de 1 Thessaloniciens 5:19-21 : « Examinez tout, retenez ce qui est bon. »"
    },
    {
  "keyword": "création",
  "synonyms": ["origine", "genèse", "formation", "naissance", "commencement", "fabrication divine", "œuvre divine", "instauration", "fondation", "émanation"],
  "response": "Dans 'La Genèse', la création est décrite comme l’acte divin par lequel Dieu donne vie à l’humanité, dotée d’une âme et d’un dessein. Le chapitre explore les récits coraniques et bibliques sur l’origine de l’univers et de l’homme. Consultez 'La Genèse' pour une analyse détaillée."
},
    {
        keywords: ["humilité", "foi", "chapitre 1"],
        response: "Le chapitre 1 de *La Voie du Salut* promeut l’humilité et la foi en s’adressant aux croyants avec respect, les invitant à méditer jour et nuit sur la parole divine, comme mentionné dans le livre de Josué, pour prospérer spirituellement."
    },
    {
        keywords: ["coran", "chapitre 1", "parole de dieu"],
        response: "Ahmed Said Aidara cite le Coran dans le chapitre 1 (Al-Qamar 54:17) : « Nous avons rendu le Coran facile à retenir. L’un de vous souhaite-t-il apprendre ? » Il encourage une méditation profonde pour intégrer les enseignements divins."
    },
    {
        keywords: ["bible", "chapitre 1", "références"],
        response: "Dans le chapitre 1, l’auteur utilise des références bibliques, comme Psaume 32:9 (« Ne soyez pas comme le cheval ou le mulet, sans intelligence ») et 1 Thessaloniciens 5:19-21, pour souligner l’importance de comprendre et vérifier la vérité."
    },
    {
        keywords: ["gratitude", "chapitre 1"],
        response: "Bien que le chapitre 1 ne mentionne pas directement la gratitude, l’auteur exprime un ton de reconnaissance envers la parole de Dieu et invite les lecteurs à une quête spirituelle empreinte de respect et d’amour."
    },
    {
        keywords: ["salut", "message principal", "livre"],
        response: "Le message principal de *La Voie du Salut* est une invitation à découvrir la voie de la paix, de la clarté et de la félicité éternelle à travers une quête spirituelle guidée par la sagesse de l’Islam et la méditation des textes sacrés."
    },
    {
        keywords: ["bonjour", "salut", "hello"],
        response: "Bonjour ! Comment puis-je vous aider à explorer les enseignements de *La Voie du Salut* ? Posez une question sur un chapitre ou un thème spécifique."
    },
    {
        keywords: ["science", "technologie", "politique"],
        response: "Je ne peux répondre qu’aux questions liées à *La Voie du Salut*. Veuillez poser une question sur le livre ou ses enseignements spirituels."
    },
    {
        keywords: ["auteur", "ahmed said aidara"],
        response: "Ahmed Said Aidara est l’auteur de *La Voie du Salut*. Il s’adresse aux croyants avec respect et amour, promouvant une quête spirituelle basée sur la foi, l’humilité et la vérification des vérités, en s’appuyant sur le Coran et la Bible."
    },
    {
  keywords: ["création"],
  response: "Le livre explique que Dieu est l’unique créateur de toute chose. L’humain a été façonné avec un but divin clair."
},
{
  keywords: ["mouhammad"],
  response: "Le Prophète Muhammad (PSL) y est présenté comme le sceau des prophètes, humble serviteur et modèle spirituel suprême."
},
{
  keywords: ["trinité"],
  response: "Le livre conteste la Trinité comme concept divin et appelle au monothéisme pur enseigné par tous les prophètes."
},
{
  keywords: ["mathématique coran"],
  response: "Des structures mathématiques étonnantes dans le Coran y sont explorées, comme la récurrence de certains mots clés."
},
{
  keywords: ["muhammad dans la bible"],
  response: "Le livre cite des versets bibliques interprétés comme des prophéties annonçant la venue de Muhammad (PSL)."
}

];

export default chatbotResponses;
